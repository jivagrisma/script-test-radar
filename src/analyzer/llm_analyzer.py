        ...
        try:
            ...
            for section in sections:
                ...
                if current_section == 'issues':
                    ...
                    suggestions.extend(line.strip('- ') for line in section.split('\n') if line.strip())
                elif current_section == 'fixes':
                    # Parse code fix blocks
                    if '```' in section:
                        original, suggested = section.split('```')[1:3]
                        fixes.append(
                            CodeFix(
                                file_path=Path('dummy.py'),  # Will be set later
                                line_start=0,  # Will be set later
                                line_end=0,  # Will be set later
                    ...
                    coverage_gaps.extend(line.strip('- ') for line in section.split('\n') if line.strip())
            
            return TestAnalysis(
                test_id='dummy',  # Will be set later
                issues=issues,
                suggestions=suggestions,
                fixes=fixes,
    ...
    async def analyze_test(
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"LLM analysis failed, falling back to local analysis: {e}")
